...
import { TOOLS_LIST_QUERY } from '../queries/ToolsQuery'
import { AGENTS_LIST_QUERY } from '../queries/AgentsQuery'
...
const Workflow = ({ loading, isNew, resetWorkflow, isDirty, workflowLens, diagram, setIsDirty, taskDefs, viewType, handleChangeView }) => {
  ...
  const [runToolsQuery, { called: toolsCalled, loading: loadingTools, data: toolsData }] = useLazyQuery(TOOLS_LIST_QUERY)
  const [runAgentsQuery, { called: agentsCalled, loading: loadingAgents, data: agentsData }] = useLazyQuery(AGENTS_LIST_QUERY)
