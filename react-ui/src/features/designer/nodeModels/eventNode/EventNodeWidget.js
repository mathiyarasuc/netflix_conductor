...
export const EventNodeWidget = ({ node, engine }) => {
  const { color, iconColor } = nodeConfig.EVENT
  ...
  return (
    <div className='event-node' style={style}>
      {/* Left Port */}
      ...
      >
        <DeviceHubIcon style={{ color: iconColor }} fontSize='small' />
      </div>

      {/* Text Section */}
      <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'left' }}>
        <span style={{ fontSize: '13px', fontWeight: 600 }}>{'EVENT'}</span>
      </div>
